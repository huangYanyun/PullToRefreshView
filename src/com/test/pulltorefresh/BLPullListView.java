...
public class BLPullListView extends ListView implements RefreshStateDelegate, BLPullDelegate{
	...
	public BLPullListView(Context context) {
		this(context,null);
	}
	
	public BLPullListView(Context context, AttributeSet attrs) {
		this(context, attrs,0);
	}
	
	public BLPullListView(Context context, AttributeSet attrs, int defStyle) {
	...
	private boolean notifyOnPulling(MotionEvent event){
		...
		if(firstItem != null && (getFirstVisiblePosition() == 0) && (firstItem.getTop() >= 0)){
			isPull = true;
